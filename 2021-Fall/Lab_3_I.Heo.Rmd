...
subtitle: "<br/>Lab 3. Hooked on a Distribution: Central Tendency, Variability, Z-Scores, and Percentiles"
...
- Sample variance = $s_X^2 = \frac{\Sigma_{i=1}^{n}(deviation\:score)^2}{n-1} =  \frac{sum\:of\:squares}{n-1} =  \frac{\Sigma_{i=1}^{n}(X_{i}-\bar{X})^2}{n-1}$

- Population variance = $\sigma_X^2 = \frac{\Sigma_{i=1}^{N}(deviation\:score)^2}{N} =  \frac{sum\:of\:squares}{N} =  \frac{\Sigma_{i=1}^{N}(X_{i}-\mu_{X})^2}{N}$

## Standard deviation

- Sample standard deviation = $\sqrt{sample\:variance} = \sqrt{s_X^2} = s_{X} =  \sqrt{\frac{\Sigma_{i=1}^{n}(X_{i}-\bar{X})^2}{n-1}}$

- Population standard deviation = $\sqrt{population\:variance} = \sqrt{\sigma_X^2} = \sigma_{X} =  \sqrt{\frac{\Sigma_{i=1}^{N}(X_{i}-\mu_X)^2}{N}}$
...
| Mode                          | The most frequent value           | The most frequent value           |
|<br>     |         |                |
| Variance                      | $s^2_X=\frac{\Sigma_{i=1}^{n}(X_{i}-\bar{X})^2}{n-1}$ | $\sigma^2_X=\frac{\Sigma_{i=1}^{N}(X_{i}-\mu_{X})^2}{N}$ |
|<br>     |         |                |
| Standard deviation            | $s_X=\sqrt{\frac{\Sigma_{i=1}^{n}(X_{i}-\bar{X})^2}{n-1}}$ | $\sigma_X=\sqrt{\frac{\Sigma_{i=1}^{N}(X_{i}-\mu_X)^2}{N}}$ |
...
## Never forget to print the Z-table for your exame :)
...
- What is $\Sigma_{1}^{10}X_{i}$ (assuming $X$ refers to the score variable)?
...
Makram is interested in programming, so he took two programming courses in R and Python, respectively. His achievement was excellent, but he is curious to compare his performances. In the R programming course, he scored 125 where the mean and the standard deviation are 80 and 18. In the Python programming course, he scored 92 where the mean and the standard deviation are 72 and 10.

## Can you answer below?
- What are the z-scores in each programming course?
- What is the percentile of his score in the R programming course?
...
- Say Ihnwhi also took the R programming course, and his z-score is 0.5.
